	...
	app.controller('MoviesController', function ( $scope, $rootScope, MovieService, $timeout ) {
		...
		.then(function (data) { //when the deferred object returns success
			$timeout(function() { //timeout to show loader, the timeout is 1000 ms, see end of function
				...
				//create an empty array to put all the genres arrays in
				var unique = [];
				//for loop that plucks out the genres array from all the objects and push it in the unique array
				for(var i=0; i < data.length; i++){
				    if(unique.indexOf(data[i].genres)){
				        unique.push(data[i].genres);
				    }
				}
				//create a new array that will contain the genres in 1 array
				var merged = [];
				//concats the unique and merged, all the different arrays inside the unique array are now transformed to 1 array (merged)
				merged = merged.concat.apply(merged, unique);
				//with underscore it removes duplicates inside the array, it is bound to genreList
				$scope.genreList = _.uniq(merged, false);
