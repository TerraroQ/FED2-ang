(function() {
	...
	angular.module('movieApp').controller('MoviesController', function ( $scope, HTTPService ) {
			HTTPService.getMovies()
				.success(function (data) {

					$scope.saved = localStorage.getItem('movieAPI');
					$scope.movieAPI = (localStorage.getItem('movieAPI')!==null) ? JSON.parse($scope.saved) : data;
					localStorage.setItem('movieAPI', JSON.stringify($scope.movieAPI));
					$scope.movies = $scope.movieAPI;


					var genre = $scope.movieAPI;
					console.log(genre);
					$scope.viewLimit = 2;
